...
- `test-template.yaml` runs for template changes. It creates a local Coder/Postgres compose deployment, gives Coder
  ...
  template with the latest released GHCR workspace image, creates a workspace, and exercises the script bundle,
  dotfiles-to-services ordering, mutable storage sizes, stop/start persistence, and deletion lifecycle.
...
neither Tailscale nor the private registry. The test uses Kind's `standard` class for both PVC types and the same
generic ephemeral `/tmp` volume the production template renders. The Compose Coder version matches the live deployment's Helm chart
...
coder template push --directory templates/kubernetes/homelab-workspace \
  ...
  --var home_pvc_storage_class=standard --var tmp_pvc_storage_class=standard \
  --var kubernetes_config_path=/home/coder/.kube/config \
  --var skip_dotfiles_scripts=true \
...
1. **Image build** — an image change exercises both published architectures and the existing private cache through
   the shared image-build workflow.
2. **Template runtime** — a template PR applies against fresh Coder, Postgres, and Kubernetes state using the last
   released GHCR image. Two service commands record if either starts before Chezmoi completes; one exits repeatedly
   and must be invoked at least twice, while the other remains running independently. The same job proves the mounted
   script bundle, both PVC size parameters, stop/start persistence, and deletion ownership rather than merely parsing
   the rendered HCL.
